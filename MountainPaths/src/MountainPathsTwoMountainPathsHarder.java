...
public class MountainPathsTwoMountainPathsHarder {
    public static void main(String[] args) throws Exception{
        ...
        int minRow = minRowCol(grid)[0][0]; // find the smallest value in col 0
        System.out.println("Row with lowest val in col 0: " + minRow);

        int minCol = minRowCol(grid)[0][1];
        System.out.println("Column with lowest val in minimum row " + minCol);
        ...
        int totalChange = drawLowestElevPath(g, grid, minRow, minCol);
        drawLowestElevPathLeft(g, grid, minRow, minCol);
...
//        totalChange = drawLowestElevPath(g, grid, bestRow, 420);
//        System.out.println("The Lowest-Elevation-Change Path starts at row: "+bestRow+" and gives a total change of: "+totalChange);
    ...
    public static int[][] minRowCol (int[][] grid) {
        int min = Integer.MAX_VALUE;
        int [][] minArray = new int[1][2];
        for (int row = 0; row < grid.length; row++) {
            for (int column = 0; column < grid[row].length; column++) {
                if (grid[row][column] < min) {
                    min = grid[row][column];
     ...
     */
    public static int drawLowestElevPath(Graphics g, int[][] grid, int row, int col){
        g.fillRect(col, row,1,1);
        int currentRow = row;
        int elevationChange = 0;
        int upfwdChange;
        int downfwdChange;
        for (int column = col; column < grid[currentRow].length - 1; column++) {
...
//                    choose upfoward
    ...
    public static int drawLowestElevPathLeft (Graphics g, int[][] grid, int row, int col){
        g.fillRect(col, row, 1, 1);
        int currentRow = row;
        int elevationChange = 0;
        int upbackChange;
        int downbackChange;
        for (int column = col; column > grid[currentRow].length; column--) {
            int backChange = Math.abs(grid[currentRow][column] - grid[currentRow][column - 1]);

//          Special Edge Case
            if (currentRow == 0) {
                upbackChange = Integer.MAX_VALUE;
            } else {
                upbackChange = Math.abs(grid[currentRow][column] - grid[currentRow - 1][column - 1]);
            }
            if (currentRow == grid.length - 1) {
                downbackChange = Integer.MAX_VALUE;
            } else {
                downbackChange = Math.abs(grid[currentRow][column] - grid[currentRow + 1][column - 1]);

            }

//          Case 1: always going forward
            if (upbackChange < backChange && upbackChange < downbackChange) {
                elevationChange+=upbackChange;
                currentRow--;
            } else if (backChange <= upbackChange && backChange <= downbackChange) {
                elevationChange+=backChange;
            } else if (downbackChange < upbackChange &&  downbackChange < backChange){
                elevationChange+=downbackChange;
                currentRow++;
            }

//          Case 2: tie w/ then go forward
            if (upbackChange == backChange || downbackChange == backChange) {
                elevationChange+=backChange;
            }

//          Case 3: bottom & top tied but less than forward, randomly flip
            if (upbackChange < backChange && downbackChange < backChange && upbackChange == downbackChange) {
                int coin = (int) (2 * Math.random());
                if (coin % 2 == 0) {
//                    choose upfoward
                    elevationChange+=upbackChange;
    ...
    public static int indexOfLowestElevPath(Graphics g, int[][] grid){
        ...
        int rowIndex = 0;
        for (int row = 0; row < grid.length; row++) {
           int currentElevPath = drawLowestElevPath(g, grid, row, 0);
